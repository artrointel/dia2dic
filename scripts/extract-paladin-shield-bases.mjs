...
const SOURCE_URL = 'https://roroyo.tistory.com/75'
const OUTPUT_PATH = 'src/data/shield-paladin-bases.json'
...
const data = {
  source: {
    title: '[디아블로2 레저렉션] 노멀/익셉셔널/엘리트 팔라딘 방패',
    url: SOURCE_URL,
  },
  category: '방패',
  type: '팔라딘 방패',
  notes: ['노멀, 익셉셔널, 엘리트 팔라딘 전용 방패의 방어력, 블럭율, 강타 피해, 최대 소켓 정리'],
  sections: tables.map((table, index) => {
    ...
    return {
      ...
      columns: ['이름', '방어력', '요구 레벨', '필요 힘', '블럭율', '강타 피해', '최대 홈'],
...
function parseTable(tableHtml) {
  ...
  return rows.map((cells) => {
    ...
    return {
      ...
      블럭율: cleanCell(cells[4] ?? '') || null,
      강타피해: parseRange(cleanCell(cells[5] ?? '')),
      전용: '팔라딘 전용',
      추천: /<b[\s>]/i.test(nameCell),
      요구레벨: parseNullableNumber(cleanCell(cells[2] ?? '')),
      필요힘: parseNullableNumber(cleanCell(cells[3] ?? '')),
