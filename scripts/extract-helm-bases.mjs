...
const SOURCE_URL = 'https://roroyo.tistory.com/101'
const OUTPUT_PATH = 'src/data/helm-bases.json'
...
const data = {
  source: {
    title: '[디아블로2 레저렉션] 노멀/익셉셔널/엘리트 투구',
    url: SOURCE_URL,
    publishedAt: '2021-11-29T20:12:47+09:00',
    modifiedAt: '2022-01-25T11:00:14+09:00',
  },
  category: '투구',
  notes: ['노멀, 익셉셔널, 엘리트 투구의 방어력, 힘제, 최대 소켓 정리'],
...
function parseTable(tableHtml, definition) {
  const rows = expandTableRows(tableHtml, definition.hasRequiredLevel ? 5 : 4)
    .filter((row) => row.length > 0)
    .slice(1)

  return rows.map((cells) => {
    const nameCell = cells[0] ?? ''
    const requiredLevelIndex = definition.hasRequiredLevel ? 2 : null
    const strengthIndex = definition.hasRequiredLevel ? 3 : 2
    const socketIndex = definition.hasRequiredLevel ? 4 : 3

    return {
      이름: normalizeItemName(cleanCell(nameCell)),
      방어력: parseRange(cleanCell(cells[1] ?? '')),
...
function normalizeItemName(value) {
  return value
    .replace(/자이언트 칸크/g, '자이언트 컨크')
    .replace(/본 비지즈/g, '본 비지즈')
}
